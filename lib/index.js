...
exports.asin = async (options) => {
    ...
    if (Array.isArray(options.asin)) {
        for (const asin of options.asin) {
            const asinOptions = { ...options, asin };
            const result = await new AmazonScraper(asinOptions).startScraper();
            data.push(result.result[0]);
        }
    } else {
        const result = await new AmazonScraper(options).startScraper();
        data = result.result;
    }
