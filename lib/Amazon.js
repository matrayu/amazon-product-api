...
class AmazonScraper {
    constructor(options) {
        this.asyncTasks = options.asyncTasks;
        this.asyncPage = 1;
        ...
        this.sponsored = options.sponsored;
        ...
        this.randomUa = options.randomUa;
        ...
        this.ua =
            options.ua || 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/81.0.4044.113 Safari/537.36';
    ...
    // Modify startScraper function in Amazon.js
    async startScraper() {
        try {
            ...
            if (this.minRating > this.maxRating) {
                throw new Error(`min rating can't be larger than max rating`);
            }
            if (this.cli) {
                spinner.start();
            }
    ...
    // Modify mainLoop method in Amazon.js
    async mainLoop() {
        if (this.scrapeType === 'asin' && Array.isArray(this.asin)) {
            for (const asin of this.asin) {
                this.asin = asin;
            ...
            return new Promise((resolve, reject) => {
                forEachLimit(
                    ...
                    async (item) => {
                        ...
                        if (this.scrapeType === 'asin') {
                            this.grabAsinDetails(body);
                            if (!this.bulk) throw new Error('Done');
                        }
                        if (this.scrapeType === 'products') {
    ...
    get setRequestEndpoint() {
        switch (this.scrapeType) {
            ...
            case 'asin':
                return `dp/${this.asin}?th=1&psc=1`;
            default:
                return '';
        }
    }
    ...
    async buildRequest(page) {
        const uri = this.setRequestEndpoint;
        const options = {
            method: 'GET',
            uri,
            qs: {
        ...
        console.log('Requesting URL:', uri); // Log the URL being requested
    ...
    async saveResultToFile() {
        if (this.collector.length) {
            switch (this.fileType) {
                case 'json':
                    await fromCallback((cb) => writeFile(`${this.fileName}.json`, JSON.stringify(this.collector, null, 2), cb));
                    break;
                case 'csv':
                ...
                case 'all':
                    await Promise.all([
                        fromCallback((cb) => writeFile(`${this.fileName}.json`, JSON.stringify(this.collector, null, 2), cb)),
                        fromCallback((cb) => writeFile(`${this.fileName}.csv`, this.jsonToCsv.parse(this.collector), cb)),
                    ]);
                    break;
                default:
                    break;
